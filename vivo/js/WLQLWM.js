 ...
 */
appMain.controller('WLQLWMCtrl',function($scope,simplePopUp,approvalFactory,$state,$ionicLoading,linkDatas,bpmCommonFunc) {
    $scope.cnNumber = cnNumber;
    var reward_selectColumn=all_selectColumn+";meetingType;div_FIsSign;" +
        "txt_FAuthor_text;txt_PMCManager_text;txt_EDManager_text;txt_QDManager_text;txt_PDManager_text;txt_AuditManager_text;txt_FieldManager_text" +
        ";txt_WarehouseManager_text;txt_FReaders_text";

    var param = {
        selectColumn: reward_selectColumn+";hidVontainer01",
    ...
    $scope.init = function(){
        ...
        for(var i= 0,l=$scope.item.approvalRecord.length;i<l;i++){
            ...
            switch ($scope.item.approvalRecord[i].ApprovalNodeName){
                ...
                case "PMC审核":$scope.item.txt_PMCManager_value =  val;break;
                case "价格审核":$scope.item.txt_EDManager_value =  val;break;
                case "审计审核":$scope.item.txt_QDManager_value =  val;break;
                case "领域审批":$scope.item.txt_PDManager_value =  val;break;
                case "总经理助理审批":$scope.item.txt_AuditManager_value =  val;break;
                case "收款确认":$scope.item.txt_FieldManager_value =  val;break;
        ...
        switch ($scope.item.txt_FDocCurrentStatus){
            ...
            case "PMC审核":$scope.item.txt_PMCManager_value =  "";break;
            case "价格审核":$scope.item.txt_EDManager_value =  "";break;
            case "审计审核":$scope.item.txt_QDManager_value =  "";break;
            case "领域审批":$scope.item.txt_PDManager_value =  "";break;
            case "总经理助理审批":$scope.item.txt_AuditManager_value =  "";break;
            case "收款确认":$scope.item.txt_FieldManager_value =  "";break;
