 ...
 */
appMain.controller('WGYBZMCtrl',function($scope,simplePopUp,approvalFactory,$state,$ionicLoading,linkDatas,bpmCommonFunc) {
    $scope.cnNumber = cnNumber;
    var lytdrm_selectColumn=all_selectColumn+";txt_FAuthor_text;txt_FManager_text;txt_FCFGManager_text;txt_FReaders_text";
    var lytdrm_encryptionColumn = "hidVontainer01";
    var param = {
    ...
    $scope.init = function(){
        ...
        for(var i= 0,l=$scope.item.approvalRecord.length;i<l;i++){
            ...
            switch ($scope.item.approvalRecord[i].ApprovalNodeName){
                ...
                case "拟稿":$scope.item.txt_FAuthor_value2 =  val;break;
    ...
    $scope.PwdValidationApprove = function(auditFlag,auditPwd,documentId,toUserName){
        if(toUserName==null){
            approvalFactory.PwdAndApprove(auditFlag,auditPwd,documentId,$scope.item.optionInput,JSON.stringify(""),function(){
                appexit(1);
            });
        }else{
