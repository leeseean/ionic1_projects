 ...
 */
appMain.controller('LDZC00Ctrl',function($scope,simplePopUp,approvalFactory,$state,$ionicLoading,linkDatas,bpmCommonFunc) {
    $scope.cnNumber = cnNumber;
    var selectColumn=all_selectColumn+";meetingType;" +
        "txt_FAuthor_text;txt_FManager_text;txt_PMCManager_text;txt_EDManager_text;txt_QDManager_text;txt_PDManager_text;txt_AuditManager_text;txt_FieldManager_text" +
        ";txt_WarehouseManager_text;txt_FReaders_text";
    ...
    $scope.init = function(){
        ...
        for(var i= 0,l=$scope.item.approvalRecord.length;i<l;i++){
            ...
            switch ($scope.item.approvalRecord[i].ApprovalNodeName){
                ...
                case "审计部处理":$scope.item.txt_FManager_value =  val;break;
                case "PMC部审核":$scope.item.txt_PMCManager_value =  val;break;
                case "工程部审核":$scope.item.txt_EDManager_value =  val;break;
                case "品质部审核":$scope.item.txt_QDManager_value =  val;break;
                case "采购部审核":$scope.item.txt_PDManager_value =  val;break;
                case "审计审核":$scope.item.txt_AuditManager_value =  val;break;
                case "领域审批":$scope.item.txt_FieldManager_value =  val;break;
                case "仓储部处理":$scope.item.txt_WarehouseManager_value =  val;break;
                default:break;
            }
        }
        switch ($scope.item.txt_FDocCurrentStatus){
            case "重新提交":$scope.item.txt_FAuthor_value =  "";break;
            case "审计部处理":$scope.item.txt_FManager_value =  "";break;
            case "PMC部审核":$scope.item.txt_PMCManager_value =  "";break;
            case "工程部审核":$scope.item.txt_EDManager_value =  "";break;
            case "品质部审核":$scope.item.txt_QDManager_value =  "";break;
            case "采购部审核":$scope.item.txt_PDManager_value =  "";break;
            case "审计审核":$scope.item.txt_AuditManager_value =  "";break;
            case "领域审批":$scope.item.txt_FieldManager_value =  "";break;
            case "仓储部处理":$scope.item.txt_WarehouseManager_value =  "";break;
            default:break;
        }
