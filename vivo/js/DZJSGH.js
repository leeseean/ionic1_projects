 ...
 */
appMain.controller('DZJSGHCtrl',function($scope,simplePopUp,approvalFactory,$state,$ionicLoading,linkDatas,bpmCommonFunc) {
    $scope.cnNumber = cnNumber;
    var dzjsgh_selectColumn =all_selectColumn+";sel_FTechnologyType;sel_FFileType;txt_FAuthor_text;txt_FManager_text;txt_FManager2_text;txt_FApprove_text;txt_FReaders_text";
    var dzjsgh_encryptionColumn = "txt_FContainer";
    var param = {
        selectColumn: dzjsgh_selectColumn+";"+dzjsgh_encryptionColumn,
        encryptionColumnn:dzjsgh_encryptionColumn,
        isGetList:false
    };
    linkDatas.getKeyAndForm(param, function (newitem) {
        newitem.sel_FTechnologyType=getSelectVal(newitem.sel_FTechnologyType);
        newitem.sel_FFileType=getSelectVal(newitem.sel_FFileType);

        var hidVontainer = decodeURIComponent(newitem.txt_FContainer);
        var txt_FContainer = hidVontainer.replace(/\+/g," ")
            .replace(/\<table/g,'<div class="ViceText-scroll"><table')
            .replace(/\<\/table>/g,'</table></div>');
        window.document.getElementById("txt_FContainer").innerHTML=txt_FContainer;
