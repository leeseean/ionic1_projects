 ...
 */
appMain.controller('CompanyInstitutionCtrl',function($scope,simplePopUp,approvalFactory,$state,$ionicLoading,linkDatas,bpmCommonFunc) {
    $scope.cnNumber = cnNumber;
    var institution_selectColum=all_selectColumn+";sel_FInstitutionCategory;txt_FAuthor_text;txt_FManager_text;txt_FCFGManager_text;txt_FApprove_text;txt_FReaders_text";
    var institution_encryptionColumn="hidVontainer01";

    var param = {
       selectColumn: institution_selectColum+";"+institution_encryptionColumn,
        encryptionColumnn:institution_encryptionColumn,
        isGetList:false
    };
    linkDatas.getKeyAndForm( param, function (newitem) {
        newitem.sel_FInstitutionCategory=getSelectVal(newitem.sel_FInstitutionCategory);
        var hidVontainer = decodeURIComponent(newitem.hidVontainer01);
        var hidVontainer01 = hidVontainer.replace(/\+/g," ")
            .replace(/\<table/g,'<div class="ViceText-scroll"><table')
            .replace(/\<\/table>/g,'</table></div>');
        window.document.getElementById("hidVontainer01").innerHTML=hidVontainer01;
    ...
    $scope.init = function() {
        ...
        for(var i= 0,l=$scope.item.approvalRecord.length;i<l;i++){
            ...
            switch ($scope.item.approvalRecord[i].ApprovalNodeName){
                ...
                case "复审":$scope.item.txt_FCFGManager_value =  val;break;
        ...
        switch ($scope.item.txt_FDocCurrentStatus){
            ...
            case "复审":$scope.item.txt_FCFGManager_value = "";break;
