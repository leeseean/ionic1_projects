 ...
 */
appMain.controller('ProdouctPlanningCtrl',function($scope,simplePopUp,approvalFactory,$state,$ionicLoading,linkDatas,bpmCommonFunc) {
    $scope.cnNumber = cnNumber;
    var productPlan_selectColumn=all_selectColumn+";sel_FProduct;txt_FAuthor_text;txt_FManager_text;txt_FCFGManager_text;txt_FReaders_text";
    var productPlan_encryptionColumn = "hidVontainer01";
    var param = {
        selectColumn: productPlan_selectColumn+";"+productPlan_encryptionColumn,
        encryptionColumnn:productPlan_encryptionColumn,
        isGetList:false
    };
    linkDatas.getKeyAndForm(param, function (newitem) {
        newitem.sel_FProduct=getSelectVal(newitem.sel_FProduct);
    ...
    $scope.init = function(){
        ...
        for(var i= 0,l=$scope.item.approvalRecord.length;i<l;i++){
            ...
            switch ($scope.item.approvalRecord[i].ApprovalNodeName){
                ...
                case "组长审核":$scope.item.txt_FManager_value2 =  val;break;
                case "部长批准":$scope.item.txt_FCFGManager_value2 =  val;break;
                default:break;
            }
        }
        switch ($scope.item.txt_FDocCurrentStatus){
            case "重新提交":$scope.item.txt_FAuthor_value2 =  "";break;
            case "组长审核":$scope.item.txt_FManager_value2 =  "";break;
            case "部长批准":$scope.item.txt_FCFGManager_value2 =  "";break;
            default:break;
        }
