 ...
 */
appMain.controller('TSQKZCCtrl',function($scope,simplePopUp,approvalFactory,$state,$ionicLoading,linkDatas,bpmCommonFunc) {
    $scope.cnNumber = cnNumber;
    var tsqkzc_selectColumn =all_selectColumn+";PersonTax;CompanyTax;LossTax;AddedValueTax;AddedValueAllTax;AddedValuePerTax;AddedValueCompanyTax"+
        ";AdditionalTax;AdditionalTaxAllTax;AdditionalPerTax;AdditionalCompanyTax;IncomeTax;IncomeAllTax;IncomePerTax;IncomeCompanyTax;OtherTax;OtherAllTax;OtherPerTax;OtherCompanyTax"+
        ";TaxLossAll;TaxLossAllPer;TaxLossAllCompany;TaxAll;TaxAllPer;TaxAllCompany;divFeesRadio;divApproveRadio;txt_FAuthor_text;txt_FManager_text;txt_FCFGManager_text;txt_AuditManager_text;txt_Review_text;txt_FReaders_text";
    var tsqkzc_encryptionColumn = "hidVontainer01";
    var param = {
        selectColumn: tsqkzc_selectColumn+";"+tsqkzc_encryptionColumn,
        encryptionColumnn:tsqkzc_encryptionColumn,
        isGetList:false
    };
    ...
    $scope.init = function(){
        ...
        for(var i= 0,l=$scope.item.approvalRecord.length;i<l;i++){
            ...
            switch ($scope.item.approvalRecord[i].ApprovalNodeName){
                case "申请人":$scope.item.txt_FAuthor_value =  val;break;
                case "重新提交":$scope.item.txt_FAuthor_value =  val;break;
                case "部长审核":$scope.item.txt_FManager_value =  val;break;
                case "审计复审":$scope.item.txt_FCFGManager_value =  val;break;
                case "审计部长审批":$scope.item.txt_AuditManager_value =  val;break;
                case "批准":$scope.item.txt_Review_value =  val;break;
                default:break;
            }
        }
        switch ($scope.item.txt_FDocCurrentStatus){
            case "重新提交":$scope.item.txt_FAuthor_value =  "";break;
            case "部长审核":$scope.item.txt_FManager_value =  "";break;
            case "审计复审":$scope.item.txt_FCFGManager_value =  "";break;
            case "审计部长审批":$scope.item.txt_AuditManager_value =  "";break;
            case "批准":$scope.item.txt_Review_value =  "";break;
            default:break;
        }
