 ...
 */
appMain.controller('YYSYJGHCtrl',function($scope,simplePopUp,approvalFactory,$state,$ionicLoading,linkDatas,bpmCommonFunc) {
    $scope.cnNumber = cnNumber;
    var selectColumn=all_selectColumn+";sel_FMachineType;txt_FBackNumber;txt_FPrimitiveIMEI;"+
    ...
    $scope.init = function(){
        ...
        for(var i= 0,l=$scope.item.approvalRecord.length;i<l;i++){
            ...
            switch ($scope.item.approvalRecord[i].ApprovalNodeName){
                ...
                case "归还收货确认":$scope.item.txt_FManager_value =  val;break;
                default:break;
            }
        }
        switch ($scope.item.txt_FDocCurrentStatus){
            case "重新提交":$scope.item.txt_FAuthor_value =  "";break;
            case "归还收货确认":$scope.item.txt_FManager_value =  "";break;
            default:break;
        }
        initScroll();
    };
