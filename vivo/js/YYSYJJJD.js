 ...
 */
appMain.controller('YYSYJJJDCtrl',function($scope,simplePopUp,approvalFactory,$state,$ionicLoading,linkDatas,bpmCommonFunc) {
    $scope.cnNumber = cnNumber;
    var selectColumn=all_selectColumn+";sel_FMachineType;sel_FPrototypeState;txt_FDescription;txt_FPrimitiveIMEI;txt_FTamperIMEI;txt_FSendNumber;txt_FTotalSendNumber;txt_FNOtBackNumber;"+
        "txt_FAuthor_text;txt_FManager_text;txt_FReaders_text";
    var param = {
    ...
    linkDatas.getKeyAndForm(param, function (newitem) {
        ...
        newitem.sel_FPrototypeState=getSelectVal(newitem.sel_FPrototypeState);
        $scope.item = newitem;
        $scope.init();
    ...
    $scope.init = function(){
        ...
        for(var i= 0,l=$scope.item.approvalRecord.length;i<l;i++){
            ...
            switch ($scope.item.approvalRecord[i].ApprovalNodeName){
                ...
                case "收货确认":$scope.item.txt_FManager_value =  val;break;
                default:break;
            }
        }
        switch ($scope.item.txt_FDocCurrentStatus){
            case "重新提交":$scope.item.txt_FAuthor_value =  "";break;
            case "收货确认":$scope.item.txt_FManager_value =  "";break;
            default:break;
        }
        initScroll();
    };
