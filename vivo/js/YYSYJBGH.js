 ...
 */
appMain.controller('YYSYJBGHCtrl',function($scope,simplePopUp,approvalFactory,$state,$ionicLoading,linkDatas,bpmCommonFunc) {
    $scope.cnNumber = cnNumber;
    var selectColumn=all_selectColumn+";sel_FMachineType;txt_FNoReturnNum;txt_FOriginalIMEICode;txt_FNoReturnReason;"+
        "txt_FAuthor_text;txt_FManager_text;txt_FCFGManager_text;txt_FDomainManager_text;txt_FAuditManager_text;txt_FAuditCancelAccount_text;txt_FReaders_text";
    var param = {
    ...
    $scope.init = function(){
        ...
        for(var i= 0,l=$scope.item.approvalRecord.length;i<l;i++){
            ...
            switch ($scope.item.approvalRecord[i].ApprovalNodeName){
                ...
                case "样机管理员审核":$scope.item.txt_FManager_value =  val;break;
                case "部长审核":$scope.item.txt_FCFGManager_value =  val;break;
                case "领域审核":$scope.item.txt_FDomainManager_value =  val;break;
                case "审计审批":$scope.item.txt_FAuditManager_value =  val;break;
                case "审计销账":$scope.item.txt_FAuditCancelAccount_value =  val;break;
                default:break;
            }
        }
        switch ($scope.item.txt_FDocCurrentStatus){
            case "重新提交":$scope.item.txt_FAuthor_value =  "";break;
            case "样机管理员审核":$scope.item.txt_FManager_value =  "";break;
            case "部长审核":$scope.item.txt_FCFGManager_value =  "";break;
            case "领域审核":$scope.item.txt_FDomainManager_value =  "";break;
            case "审计审批":$scope.item.txt_FAuditManager_value =  "";break;
            case "审计销账":$scope.item.txt_FAuditCancelAccount_value =  "";break;
            default:break;
        }
        initScroll();
    };
