...
/*var processInstId = {};*/
ionicApp.controller('auditType2Control',function($scope,$http,$stateParams,$state,$window,$ionicPopover,$ionicGesture) {

    var id = $stateParams.id;
    $scope.back = function () {
        $state.go("home.showHis");
        //$window.history.back();
    };
    $scope.data=eval("(" + localStorage.getItem("audit") + ")");
    $scope.element = angular.element(document.querySelector('#auditType2'));
    $scope.events = [{
        event: 'swipeup'
    }, {
        event: 'swipedown'
        ...
        $ionicGesture.on(obj.event, function () {
            if(obj.event == "swiperight"){
                $scope.$apply(function(){
                    $state.go('auditHis2',{id:$stateParams.id});
                });
                    ...
                    $http.post(serverUrl+"com.cms.formpatch.formMobile.queryFormWFInst.biz.ext", {
                      ...
                      /*  var u = navigator.userAgent;
                        ...
                        if (isAndroid){
                           ...
                           // alert("kk");
                            $state.go('auditFormDetial2',{Url:JSON.stringify(toUrl),id:$stateParams.id});
                       /* }*/
                    });
                });
            }
