 ...
 */
/*====================================转办==============================================*/
ionicApp.controller('pgAuditAdvise3Control',function($scope,$http,$stateParams,$state,$window,showAlert,$ionicLoading,localStorageService){
    var inf3 = eval("("+$stateParams.list3+")");
    var list3realname = eval("("+$stateParams.list3realname+")");//选择转办人传过来的数据
    ...
    $scope.choose = function(){
        $state.go('lookUp',{list4:JSON.stringify(inf3)});
    };
    $scope.realName=[];
 ...
 /*   for(var x in list3realname){
    ...
    $scope.turnTodo=function() {
        ...
        $http.post(serverUrl+"com.smap.wfajax.disp.handle.consultingEx2.biz.ext",
            {
                "advise":advise,
