...
/*var processInstId = {};*/
ionicApp.controller('auditHis2Control',function($scope,$http,$stateParams,$state,$window,$ionicPopover,$ionicGesture) {
    ...
    $scope.element = angular.element(document.querySelector('#auditHis2'));
    $scope.events = [{
        event: 'swipeup'
    }, {
        event: 'swipedown'
        ...
        $ionicGesture.on(obj.event, function () {
            if(obj.event == "swiperight"){
                $scope.$apply(function(){
                    $state.go('detail2',{id:$stateParams.id});
                });
            }else if(obj.event == "swipeleft"){
                $scope.$apply(function(){
                    $state.go('auditType2',{id:$stateParams.id});
                });
            }
        }, $scope.element);
    });
