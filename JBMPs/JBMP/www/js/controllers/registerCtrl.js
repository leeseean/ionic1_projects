...
app.controller('RegisterCtrl', ['$scope','$state', 'Api', 'JingboServer', '$stateParams', '$timeout','$ionicPopup', '$ionicLoading',
    function ($scope,$state, Api, JingboServer, $stateParams, $timeout, $ionicPopup, $ionicLoading) {
      ...
      $scope.checkBind = function(){
        ...
        var params = {'openId':hex_sha1(openId)};
        JingboServer.getData(
          Api.getOpenIdLogin,
          'POST',
          params,
          function(res){
             $state.go('sucBind',{dataParams:{UserName:res.data['UserName'],CustomerName:res.data['CustomerName']}});
          },
          function(err){
      ...
      $scope.signIn = function(u){
        ...
        JingboServer.getData(
          ...
          function (res) {
            ...
            $state.go('sucBind',{dataParams:{UserName:res.data['UserName'],CustomerName:res.data['CustomerName']}}); //add 6.16
          },
          function (err) {
